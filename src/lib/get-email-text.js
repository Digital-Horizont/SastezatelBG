...
export function getEasyPayCustomerEmailText(product_name , payment_type , result , email , phone , description , quantity){
  return `
    ...
    Дължима сума: €${(result.amount / EUR_TO_BGN).toFixed(2)}
...
export function getEasyPayAdminEmailText(product_name , payment_type , result , email , phone , description , quantity){
  return `
    ...
    ${payment_type !== 'merch' && payment_type !== 'book' ? 'Брой месеци' : 'Количество продукти'}: ${quantity}
    Обща Цена: €${(result.amount / EUR_TO_BGN).toFixed(2)}
    Email: ${email}
...
export function getBankCustomerEmailText(payment_type , product_name , amount , email , phone , description , quantity){
  return `
    ...
    Цена: € ${(amount / EUR_TO_BGN).toFixed(2)}
...
export function getBankAdminEmailText(payment_type , product_name , amount , email , phone , description , quantity){
  return   `
    ...
    Продукт: ${product_name}
    ${payment_type !== 'merch' && payment_type !== 'book' ? 'Брой месеци' : 'Количество продукти'}: ${quantity}
    Обща Цена: € ${(amount / EUR_TO_BGN).toFixed(2)}
    Email: ${email}
    Телефон: ${phone}
